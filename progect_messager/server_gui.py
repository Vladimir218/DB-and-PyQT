...
class ConfigWindow(QDialog):
    ...
    def initUI(self):
        ...
        self.port_label.setFixedSize(175, 30)
        ...
        self.ip_label.setFixedSize(180, 30)

        # Метка с напоминанием о пустом поле.
        self.ip_label_note = QLabel(' оставьте это поле пустым, чтобы\n принимать соединения с любых адресов.', self)

        self.ip_label_note.move(6, 182)
        self.ip_label_note.setFixedSize(500, 32)
...
if __name__ == '__main__':
    ...
    main_window.statusBar().showMessage(f'Test Statusbar Message {datetime.now()}')
    test_list = QStandardItemModel(main_window)
    test_list.setHorizontalHeaderLabels(['Имя Клиента', 'IP Адрес', 'Порт', 'Время подключения'])
    for i in range(1,50):
        test_list.appendRow(
            [QStandardItem(f'test{i}'), QStandardItem(f'192.198.0.{i}'), QStandardItem(f'{23544+i}'), QStandardItem(f'{i}:20:34')])
   ...
   # app = QApplication(sys.argv)
    ...
    for i in range(1,50):
        test_list.appendRow(
            [QStandardItem(f'test{i}'), QStandardItem(f'Fri Dec {i} 16:20:34 2020'), QStandardItem(f'{i}'), QStandardItem(f'{i+1}')])
